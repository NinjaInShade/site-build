...
export default function ControlPanel() {
  ...
  useEffect(() => {
    console.log(`Send fetch request to get data for user #${userid} sites`);
    console.log("Save this data to the sites state and send it down to any component that needs it, like siteDashboard.");
  ...
  return (
    <section className="ControlPanel">
      ...
      <section>
        ...
        <Switch>
          <ProtectedRoute path={`${path}/site/:siteid`} component={SiteDashboard} user={userData} sites={sites} />
          <ProtectedRoute path={`${path}/`} component={SitesOverview} exact sites={sites} user={userData} />
        </Switch>
      </section>
    </section>
  );
